...
export default function ProviderSignupPage() {
  ...
  async function submit(event) {
    ...
    try {
      const application = {
        ...form,
        servicesOffered: withoutOptions(form.servicesOffered, HIDDEN_SERVICE_OPTIONS),
      };
...
function Care({ form, change, toggle, addOther, options }) {
  return <Section title="Areas of care" text="Help people understand who you support, how you work, and where you can serve clients.">
    ...
    <MultiSelect label="General Availability" values={form.availability} options={options.availability} onToggle={(value) => toggle("availability", value)} allowCustom={false} required />
    ...
    <Row><TextField label="Price" value={form.price} onChange={change("price")} required placeholder="$125/session, sliding scale, varies..." /><TextField label="Physical locations" value={form.physicalLocations} onChange={change("physicalLocations")} required placeholder="Philadelphia, South Jersey, virtual only..." /></Row>
  </Section>;
}
...
function Collaboration({ form, change, toggle, addOther, options }) {
  return <Section title="Collaboration" text="Tell aligned providers how you like to connect, collaborate, and build community.">
...
function Human({ form, change, toggle, options }) {
  return <Section title="The human side" text="Add the warmth and perspective that help people get a feel for you beyond credentials.">
...
function Consent({ form, change, setValue }) {
  return <Section title="Consent" text="Confirm that we can review and publish your approved provider information.">
...
function PhotoUpload({ value, onChange }) {
  ...
  return <label className="provider-field provider-full provider-file-field">
    ...
    {value?.name ? <small>Selected: {value.name}</small> : <small>Upload a headshot or profile image. Large photos will be resized before submitting.</small>}
